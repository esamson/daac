...
object Main {
  ...
  private val SplitSize = 1L << 17
  private val ProgressInterval = SECONDS.toMillis(10)
  private val SmoothingFactor = BigDecimal("0.05")
  private val MaxBackoffMs = 10000
  private val BackoffSlotMs = 100
  private val MaxBackoffSlots = LazyList
    .from(1)
    .filter(n => (pow(2, n).toInt - 1) * BackoffSlotMs > MaxBackoffMs)
    .head
  private val cacheDir = Paths.get(sys.props("user.home"), "Downloads", ".daac")
      ...
      if (Files.exists(destFile) && Files.size(destFile) == contentSize) {
        ...
        val progressReporter = new Thread(() => {
          ...
          while (progressAccumulator.get() < contentSize) {
            try Thread.sleep(ProgressInterval)
            catch {
  ...
  def cleanUpCacheDir(): Unit = {
    ...
    if (Files.exists(cacheDir)) {
      ...
      if (cacheDir.toFile.listFiles().isEmpty) Files.delete(cacheDir)
    }
  }

  case class DownloadSpec(
      url: String,
